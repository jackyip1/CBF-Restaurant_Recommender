    ...
    handleResponseFromWatsonConversation(message, user, conversationResponse) {
        ...
        return this.getOrCreateActiveConversationId(user, conversationResponse)
            .then(() => {
                ...
                const datefixed = conversationResponse.context.datefixed;
                const venuefixed = conversationResponse.context.venuefixed;
                var friendname = conversationResponse.context.venuefixed;
                if (action == "askingfriend") {
                    conversationResponse.context.action = null;
                    return Promise.resolve({
                        text: "We're sending a message to Jack to confirm.",
                        otherUser: "@jackyip",
                        otherUserText: "Your friend wants to meet you at " + venuefixed + " on " + datefixed +"?",
                    });
                    ...
                    return Promise.resolve({
                        ...
                        otherUser: "@karan",
                        otherUserText: "Jack confirmed the date."
                    });
                }
                else {
    ...
    handleFindDoctorByLocationMessage(conversationResponse) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.foursquareClient.getVenues(params, function(error, venues) {
                ...
                if (error) {
                    console.log(error);
                    reply = 'Lemme ask your friend if he is free and get back to you.';
                }
                else {

                ...
                if (!data.bot_id) {
                    ...
                    this.healthBot.processMessage(messageSender, message)
                        .then((reply) => {
                            ...
                            if (reply.text.otherUser) {
                                console.log(reply.text.otherUser);
                                console.log(reply.text.otherUserText);
                                slackBot.postMessage(data.channel, reply.text.text, {});
                                if (reply.text.otherUser == "@jackyip") {
                                slackBot.postMessage('@jackyip', reply.text.otherUserText, {as_user:false});
                                }
                                else
                                    slackBot.postMessage('@karan', reply.text.otherUserText, {as_user:false});
                            }
                            else {
                                slackBot.postMessage(data.channel, reply.text, {});
                            }
